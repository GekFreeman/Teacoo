...
public class AcceptPop extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AcceptPop frame = new AcceptPop(applyID);
	 ...
	 * Create the frame.
	 */
	public AcceptPop(String applyID) {
		//this.userID=userID;
		setResizable(false);
		// 去掉窗口的装饰
		setUndecorated(true);
		...
		container=new JPanel(){protected void paintComponent(Graphics g) {
			ImageIcon BG = new ImageIcon(Toolkit.getDefaultToolkit().getImage("image/management/acceptpop.png"));
	...
	//将用户申请的题目加入数据库中
	public void insertToDB(String applyid){
		GetDataFromDB data=new GetDataFromDB();
		
		
		data.getConnection();
		data.getText(applyid);
		data.getAnswer(applyid);
		
		UpdateDB.insert(data.main,data.oA,data.oB,data.oC,data.oD,data.answer,"Multiple choice");
