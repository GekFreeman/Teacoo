...
public class DeleteUserPop extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					DeleteUserPop frame = new DeleteUserPop(userID);
	 ...
	 * Create the frame.
	 */
	public DeleteUserPop(String userID) {
		this.userID=userID;
	setResizable(false);
	// 去掉窗口的装饰
	setUndecorated(true);
	...
	container=new JPanel(){protected void paintComponent(Graphics g) {
		ImageIcon BG = new ImageIcon(Toolkit.getDefaultToolkit().getImage("image/management/deletepop.png"));
	...
	btnY.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			UpdateDB a=new UpdateDB();//从数据库中删除该用户
			a.deleteuser(userID);
			dispose();
		}
	});
